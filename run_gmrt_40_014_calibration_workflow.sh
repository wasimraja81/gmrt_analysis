...
usage() {
  cat <<'USAGE'
Usage:
  bash run_gmrt_40_014_calibration_workflow.sh            # run full 10-step workflow
  bash run_gmrt_40_014_calibration_workflow.sh --audit-only
...
# 1) clean
run_step 1 "clean work outputs" bash clean_work_outputs.sh --yes

# 2) primary cal
run_step 2 "primary calibration" bash primaryCalibration_example.sh
assert_file "$PRIMARY_BANDPASS"
assert_file "$PRIMARY_FLAGS"

# 3) split primary itself (3C48 with primary corrections + primary flags)
run_step 3 "split 3C48 with primary calibration+flags" bash visSplit_3c48_example.sh
assert_file "$SPLIT_3C48"

# 4) plot visibilities for primary cal
run_step 4 "plot visibilities for primary calibration" bash plotVis_3c48_example.sh
assert_file "$PLOT_3C48_PDF"

# 5) split 3c468.1 (applying primary-only corrections and flags)
run_step 5 "split 3C468.1 with primary-only corrections+flags" bash visSplit_3c468.1_primary_example.sh
assert_file "$SPLIT_3C468_PRIMARY"

# 6) plot vis for step-5
run_step 6 "plot visibilities for primary-only 3C468.1 split" bash plotVis_3c468.1_primary_example.sh
assert_file "$PLOT_3C468_PRIMARY_PDF"
...
run_step 7 "derive clustering flags for 3C468.1" bash clustering_3c468.1_split_example.sh
assert_file "$CLUSTER_FLAGS"
assert_glob "$CLUSTER_PRODUCTS_GLOB"

# 8) derive secondary calibration using new clustering flags
run_step 8 "secondary calibration using clustering flags" bash secondaryCalibration_example.sh
assert_glob "$SECONDARY_TABLES_GLOB"

# 9) generate primary+secondary calibrated 3c468.1
run_step 9 "split 3C468.1 with primary+secondary calibration" bash visSplit_3c468.1_example.sh
assert_file "$SPLIT_3C468_FINAL"

# 10) plot vis on step-9 output
run_step 10 "plot visibilities for primary+secondary 3C468.1 split" bash plotVis_3c468.1_example.sh
assert_file "$PLOT_3C468_FINAL_PDF"
