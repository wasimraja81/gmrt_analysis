...
def _import_ugmrt():
    """Import (or reload) modules.ugmrt_query from this repository layout."""
    import importlib, sys
    script_dir = Path(__file__).resolve().parent
    module_name = 'modules.ugmrt_query'
    if str(script_dir) not in sys.path:
        sys.path.insert(0, str(script_dir))
    if module_name in sys.modules:
        return importlib.reload(sys.modules[module_name])
    import modules.ugmrt_query as ugmrt_query
    return ugmrt_query
...
def _run_final_clustering_step(q, workflow_result: dict, dry_run: bool) -> dict:
    ...
    _od_path = Path(__file__).resolve().parent / 'modules' / 'outlier_detection.py'
    if not _od_path.exists():
        raise FileNotFoundError(
            f'outlier_detection.py not found under src/modules: {_od_path}'
        )
...
def main():
    ...
    _default_cfg = str(Path(__file__).resolve().parent.parent / 'preprocess_ugmrt.cfg')
